...
instance_ip = response['Reservations'][0]['Instances'][0].get('PublicIpAddress')
try:
    print(f"Copying monitoring.sh to {instance_id} at {instance_ip}...")
    subprocess.run(["scp", "-i", f"{keypair}.pem", "monitoring.sh", f"ec2-user@{instance_ip}:."], check=True)
    print(f"Connecting to {instance_id} at {instance_ip}...")
    subprocess.run(["ssh", "-i", f"{keypair}.pem", f"ec2-user@{instance_ip}", "chmod 700 monitoring.sh"], check=True)
    print(f"Running...")
    subprocess.run(["ssh", "-i", f"{keypair}.pem", f"ec2-user@{instance_ip}", "./monitoring.sh"], check=True)
